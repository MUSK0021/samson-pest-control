/* Samson Pest Control: site interactions.
   Needs gsap + ScrollTrigger + SplitText (loaded before this file). Uses native browser scrolling.
   Everything degrades
...
(function () {
  ...
  /* ---------------------------------------------------------------- scrolling (native, no smoothing library) */
  ...
  var docH = html.scrollHeight, viewH = window.innerHeight, ticking = false;
  var state = {};
  function setClass(el, name, on) {
    if (!el || state[name] === on) return;
    state[name] = on;
    el.classList.toggle(name.split(':')[1], on);
  }
  function measure() { docH = html.scrollHeight; viewH = window.innerHeight; }

  // runs at most once per frame, and only touches the DOM when something actually changes
  function onScroll() {
    ticking = false;
    if (!hdr) return;
    var y = window.scrollY;
    setClass(hdr, 'hdr:is-stuck', y > 30);
    if (y > 520 && y > lastY + 4 && !menuOpen && !hdr.matches(':focus-within')) setClass(hdr, 'hdr:is-hidden', true);
    else if (y < lastY - 4 || y < 520) setClass(hdr, 'hdr:is-hidden', false);
    lastY = y;
    var max = docH - viewH;
    if (progress) progress.style.transform = 'scaleX(' + (max > 0 ? Math.min(1, y / max).toFixed(4) : 0) + ')';
    if (quickbar) setClass(quickbar, 'qb:is-on', y > viewH * 0.7 && y + viewH < docH - 420 && !menuOpen);
  }
  ...
  if (hasGsap) ST.addEventListener('refresh', measure);
  onScroll();
  ...
  safe('menu', function () {
    ...
    function setMenu(open, returnFocus) {
      ...
      setClass(hdr, 'hdr:is-hidden', false);
      setClass(quickbar, 'qb:is-on', false);
      if (open) {
        menu.hidden = false;
  ...
  safe('wipe', function () {
    ...
    d.addEventListener('click', function (e) {
      ...
      gsap.fromTo(wipe, { yPercent: 101, y: 0 }, { yPercent: 0, duration: 0.4, ease: 'power3.inOut', onComplete: function () { location.href = url.href; } });
    });
  ...
  safe('pencil', function () {
    ...
    var tl = gsap.timeline({
      defaults: { ease: 'none' },
      scrollTrigger: { trigger: pencil, start: 'top top', end: mobile ? '+=260%' : '+=340%', pin: true, scrub: 0.35, anticipatePin: 1 }
    });
  ...
  safe('seasons', function () {
    ...
    mm.add('(min-width: 900px)', function () {
      ...
      var tween = gsap.to(track, {
        ...
        scrollTrigger: { trigger: sec, start: 'top top', end: function () { return '+=' + dist(); }, pin: true, scrub: 0.35, invalidateOnRefresh: true, anticipatePin: 1 }
      });
  ...
  safe('marquee', function () {
    ...
    if (!track || reduce || !hasGsap || !track.getAnimations) return;
    ...
    ST.create({
      ...
      onUpdate: function (self) {
        ...
        anim.playbackRate = 1 + Math.min(Math.abs(self.getVelocity()) / 600, 4);
        clearTimeout(timer);
        timer = setTimeout(function () { gsap.to(anim, { playbackRate: 1, duration: 0.8, ease: 'power2.out' }); }, 120);
      }
    });
  });

  /* ---------------------------------------------------------------- pointer effects (subtle: the cursor stays the normal system cursor) */
  safe('glow', function () {
    if (!fine) return;
  ...
  safe('hover-img', function () {
    ...
    var xTo = gsap.quickTo(box, 'x', { duration: 0.35, ease: 'power3.out' });
    var yTo = gsap.quickTo(box, 'y', { duration: 0.35, ease: 'power3.out' });
    window.addEventListener('pointermove', function (e) {
      xTo(e.clientX + 230); yTo(e.clientY);
    }, { passive: true });
  ...
  safe('gline', function () {
    var g = $('[data-gline]');
    if (!g || !animOn) return;
    gsap.timeline({ scrollTrigger: { trigger: g, start: 'top 82%', end: 'bottom 45%', scrub: 0.35 } })
